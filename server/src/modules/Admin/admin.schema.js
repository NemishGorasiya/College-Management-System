...
export const adminUpdateSchema = {
  body: Joi.object({
    email: Joi.string().email().optional().messages({ "string.email": "Email is required" }),
    phoneNumber: Joi.string().optional().messages({ "any.required": "Phone number is required" }),
    address: Joi.string().optional().messages({ "any.required": "Address is required" }),
    dob: Joi.date().optional().max('now').messages({ "date.max": "Date of birth is required and must be in the past" }),
    doj: Joi.date().optional().max('now').messages({ "date.max": "Date of joining is required and must be in the past" }),
    firstName: Joi.string().optional().messages({ "any.required": "First name is required" }),
